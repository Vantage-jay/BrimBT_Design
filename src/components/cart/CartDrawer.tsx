...
import { useCartStore, useFormattedSubtotal } from '@/store/cartStore'
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[9px] tracking-[0.2em] uppercase text-brand-gray mb-0.5">
          {product.brand}
        </p>
        <Link
          href={`/products/${product.slug}`}
          className="font-display text-[14px] text-brand-white hover:text-brand-accent transition-colors leading-snug block mb-1"
        >
          {product.name}
        </Link>
        <p className="text-[10px] text-brand-gray mb-3">
          Size: <span className="text-brand-white">{selectedSize}</span>
        </p>

        <div className="flex items-center justify-between">
          {/* Qty controls */}
          <div className="flex items-center border border-brand-accent/20 h-8">
            <button
              onClick={() => updateQuantity(product.id, selectedSize, quantity - 1)}
              className="w-8 h-full flex items-center justify-center text-brand-gray hover:text-brand-white transition-colors"
            >
              <Minus size={11} />
            </button>
            <span className="w-6 text-center text-[12px] text-brand-white">{quantity}</span>
            <button
              onClick={() => updateQuantity(product.id, selectedSize, quantity + 1)}
              className="w-8 h-full flex items-center justify-center text-brand-gray hover:text-brand-white transition-colors"
            >
          ...
          <span className="text-[14px] text-brand-white font-light">{linePrice}</span>
        </div>
      </div>
      ...
      <button
        ...
        className="text-brand-gray hover:text-red-400 transition-colors self-start mt-0.5 flex-shrink-0"
      >
        <Trash2 size={14} />
      </button>
    </motion.div>
...
function EmptyCart({ onClose }: { onClose: () => void }) {
  return (
    <div className="flex-1 flex flex-col items-center justify-center py-16 text-center px-6">
      ...
      <p className="font-display text-2xl font-light text-brand-white mb-3">Your cart is empty</p>
      <p className="text-[13px] text-brand-gray leading-relaxed mb-10 max-w-[200px]">
...
function ShippingProgress({ subtotal }: { subtotal: number }) {
  ...
  return (
    <div className="px-6 py-4 border-b border-brand-accent/12 bg-brand-accent/[0.03]">
...
function OrderSummary({ subtotal, closeCart }: { subtotal: number; closeCart: () => void }) {
  ...
  return (
    <div className="border-t border-brand-accent/15 px-6 pt-5 pb-6">
      {/* Line items */}
      <div className="space-y-2 mb-4">
        {[
          ...
          <div key={label} className="flex justify-between text-[12px]">
            ...
            <span className={accent ? 'text-green-400' : 'text-brand-white'}>{value}</span>
          </div>
        ))}
      </div>
      ...
      <p className="text-[10px] text-brand-gray mb-5">Tax calculated at checkout. Final amount may vary.</p>
      ...
      <button
        onClick={closeCart}
        className="w-full text-center text-[11px] tracking-[0.15em] uppercase text-brand-gray hover:text-brand-white transition-colors py-1"
      >
        Continue Shopping
      </button>
    </div>
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-0 right-0 h-full w-full sm:w-[420px] bg-brand-black border-l border-brand-accent/20 z-50 flex flex-col shadow-2xl"
            role="dialog"
            aria-label="Shopping cart"
            aria-modal="true"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-5 border-b border-brand-accent/15">
              <div className="flex items-center gap-3">
                <ShoppingBag size={18} className="text-brand-accent" />
                <span className="font-display text-lg text-brand-white">Your Cart</span>
                {totalItems > 0 && (
              ...
              <button onClick={closeCart} aria-label="Close cart" className="text-brand-gray hover:text-brand-white transition-colors">
                <X size={20} />
              </button>
            </div>
