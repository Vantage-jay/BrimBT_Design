    ...
    <div>
      <div className="flex border border-brand-accent/30 focus-within:border-brand-accent/60 transition-colors">
        <input
          ...
          className="flex-1 bg-transparent px-4 py-3 text-sm text-brand-black placeholder:text-brand-text-secondary focus:outline-none"
        />
        <button
...
export default function Footer() {
  ...
  return (
    <footer className="border-t border-brand-accent/20 bg-brand-surface">
      ...
      <div className="max-w-[1440px] mx-auto px-15 pt-20 pb-14">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[2fr_1fr_1fr_1fr] gap-14">
          ...
          <div>
            ...
            <Link
              href="/"
              className="font-display text-3xl font-semibold tracking-[0.22em] text-brand-black uppercase block mb-5"
            >
              Brim<span className="text-brand-accent">BT</span> Design
            </Link>

            {/* Tagline */}
            <p className="text-[13px] text-brand-text-secondary leading-[1.9] max-w-[280px] mb-8">
            ...
            <div className="flex items-center gap-5">
              {SOCIAL_LINKS.map((s) => (
                <a
                  ...
                  aria-label={s.label}
                  className="text-[10px] tracking-[0.2em] uppercase text-brand-text-secondary hover:text-brand-accent transition-colors"
                >
                  {s.label}
                </a>
              ))}
              ...
              <ul className="space-y-3">
                {col.links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-[13px] text-brand-text-secondary hover:text-brand-black transition-colors"
                    >
                      {link.label}
                    </Link>
                  </li>
      ...
      {/* ── Divider ──────────────────────────────────────── */}
      <div className="border-t border-brand-accent/15" />

      {/* ── Bottom Bar ───────────────────────────────────── */}
      <div className="max-w-[1440px] mx-auto px-15 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">

        {/* Copyright */}
        <p className="text-[12px] text-brand-text-secondary tracking-wide">
        ...
        <div className="flex items-center gap-6">
          {LEGAL_LINKS.map((link, i) => (
            <span key={link.href} className="flex items-center gap-6">
              <Link
                href={link.href}
                className="text-[11px] text-brand-text-secondary hover:text-brand-black tracking-wide transition-colors"
              >
                {link.label}
              </Link>
        ...
        <div className="flex items-center gap-3">
          {['Visa', 'Mastercard', 'PayPal', 'Stripe'].map((p) => (
            <span
              key={p}
              className="text-[9px] tracking-[0.1em] uppercase text-brand-text-secondary/50 border border-brand-gray/20 px-2 py-1"
            >
              {p}
            </span>
          ))}
