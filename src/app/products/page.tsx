...
/* ============================================================
   BrimBT Design — Product Listings Page
...
const ALL_PRODUCTS: Product[] = [
  ...
  { id:'5',  name:'Ribbed Bodysuit',         brand:'BrimBT Studio',   slug:'ribbed-bodysuit',         category:'clothing',    description:'Seamless ribbed bodysuit.',        price:89,  badge:'new',  images:[{url:'',alt:''}], variants:[{size:'XS',stock:6},{size:'S',stock:10},{size:'M',stock:8},{size:'L',stock:4}],  rating:4.6, reviewCount:58,  isFeatured:false, isNew:true,  createdAt:'2026-01-15' },
  { id:'6',  name:'Platform Chelsea Boot',   brand:'BrimBT Footwear', slug:'platform-chelsea-boot',   category:'footwear',    description:'Chunky platform Chelsea boot.',    price:198, badge:null,   images:[{url:'',alt:''}], variants:[{size:'37',stock:4},{size:'38',stock:6},{size:'39',stock:5},{size:'40',stock:3}], rating:4.7, reviewCount:43,  isFeatured:false, isNew:false, createdAt:'2026-01-18' },
  { id:'7',  name:'Oversized Blazer',        brand:'BrimBT Studio',   slug:'oversized-blazer',        category:'clothing',    description:'Structured oversized blazer.',     price:265, badge:null,   images:[{url:'',alt:''}], variants:[{size:'S',stock:3},{size:'M',stock:5},{size:'L',stock:4},{size:'XL',stock:2}],  rating:4.8, reviewCount:76,  isFeatured:false, isNew:false, createdAt:'2026-01-20' },
  ...
  { id:'10', name:'Seamless Bike Shorts',    brand:'BrimBT Studio',   slug:'seamless-bike-shorts',    category:'essentials',  description:'High-waist seamless bike shorts.', price:64,  originalPrice:85, badge:'sale', images:[{url:'',alt:''}], variants:[{size:'XS',stock:8},{size:'S',stock:10},{size:'M',stock:7},{size:'L',stock:5}],  rating:4.6, reviewCount:112, isFeatured:false, isNew:false, createdAt:'2026-01-26' },
  { id:'11', name:'Strappy Sandal',          brand:'BrimBT Footwear', slug:'strappy-sandal',          category:'footwear',    description:'Minimalist strappy flat sandal.',  price:118, badge:null,   images:[{url:'',alt:''}], variants:[{size:'36',stock:6},{size:'37',stock:8},{size:'38',stock:5},{size:'39',stock:4}], rating:4.4, reviewCount:29,  isFeatured:false, isNew:false, createdAt:'2026-01-28' },
  { id:'12', name:'Knit Midi Dress',         brand:'BrimBT Studio',   slug:'knit-midi-dress',         category:'clothing',    description:'Fine-knit midi dress with slit.',  price:178, badge:'limited', images:[{url:'',alt:''}], variants:[{size:'XS',stock:2},{size:'S',stock:3},{size:'M',stock:2},{size:'L',stock:1}], rating:4.8, reviewCount:55,  isFeatured:false, isNew:false, createdAt:'2026-01-30' },
]
...
const ALL_SIZES    = ['XS','S','M','L','XL','36','37','38','39','40','41','One Size']
const PAGE_SIZE    = 8
...
function applyFilters(products: Product[], filters: ProductFilters): Product[] {
  ...
  if (filters.sizes.length > 0)
    result = result.filter((p) => p.variants.some((v) => filters.sizes.includes(v.size) && v.stock > 0))
  ...
  return (
    <div className="border-b border-brand-accent/12 py-5">
      <button onClick={() => setOpen((o) => !o)} className="flex items-center justify-between w-full">
        <span className="text-[11px] tracking-[0.25em] uppercase text-brand-white font-medium">{title}</span>
...
function FilterSidebar({ filters, onChange, onClear, totalActive }: {
  filters: ProductFilters; onChange: (f: ProductFilters) => void; onClear: () => void; totalActive: number
}) {
  const update = <K extends keyof ProductFilters>(key: K, value: ProductFilters[K]) =>
    onChange({ ...filters, [key]: value })
  const toggleSize = (size: string) => {
    const sizes = filters.sizes.includes(size) ? filters.sizes.filter((s) => s !== size) : [...filters.sizes, size]
    update('sizes', sizes)
  }
  return (
    <aside className="w-full lg:w-[240px] flex-shrink-0">
      <div className="flex items-center justify-between mb-2 pb-5 border-b border-brand-accent/12">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] tracking-[0.25em] uppercase text-brand-white font-medium">Filters</span>
          {totalActive > 0 && (
            <span className="w-5 h-5 rounded-full bg-brand-accent text-brand-black text-[9px] font-medium flex items-center justify-center">{totalActive}</span>
          )}
        </div>
        {totalActive > 0 && (
          <button onClick={onClear} className="text-[10px] tracking-[0.15em] uppercase text-brand-gray hover:text-brand-accent transition-colors">Clear all</button>
        )}
      </div>
        ...
        <div className="space-y-2">
          {CATEGORIES.map((cat) => (
            <button key={cat.value} onClick={() => update('category', cat.value)}
              className={clsx('block w-full text-left text-[12px] tracking-wide py-1 transition-colors',
                filters.category === cat.value ? 'text-brand-accent' : 'text-brand-gray hover:text-brand-white'
              )}>
              {cat.label}
            </button>
          ))}
      ...
      <FilterSection title="Price Range">
        <div className="space-y-3">
          ...
          <input type="range" min={0} max={500} step={10} value={filters.priceRange[1]}
            onChange={(e) => update('priceRange', [filters.priceRange[0], Number(e.target.value)])}
            className="w-full accent-brand-accent cursor-pointer"
          />
          <div className="flex gap-2">
            {[100,200,300,500].map((max) => (
              <button key={max} onClick={() => update('priceRange', [0, max])}
                className={clsx('text-[9px] px-2 py-1 border transition-colors',
                  filters.priceRange[1] === max ? 'border-brand-accent text-brand-accent' : 'border-white/10 text-brand-gray hover:border-brand-accent hover:text-brand-accent'
                )}>
                &lt;${max}
              </button>
            ))}
        ...
        <div className="flex flex-wrap gap-2">
          {ALL_SIZES.map((size) => (
            <button key={size} onClick={() => toggleSize(size)}
              className={clsx('text-[9px] px-2.5 py-1.5 border transition-colors',
                filters.sizes.includes(size) ? 'border-brand-accent text-brand-accent bg-brand-accent/10' : 'border-white/10 text-brand-gray hover:border-brand-accent hover:text-brand-accent'
              )}>
              {size}
            </button>
          ))}
      ...
      <FilterSection title="Collection Tag">
        <div className="space-y-2">
          {BADGES.map((b) => (
            <button key={b.value} onClick={() => update('badge', b.value as ProductFilters['badge'])}
              className={clsx('block w-full text-left text-[12px] tracking-wide py-1 transition-colors',
                filters.badge === b.value ? 'text-brand-accent' : 'text-brand-gray hover:text-brand-white'
              )}>
              {b.label}
            </button>
          ))}
...
function ActiveChips({ filters, onChange }: { filters: ProductFilters; onChange: (f: ProductFilters) => void }) {
...
function SortBar({ sort, onSort, showMobileFilters, activeFilterCount }: {
  sort: SortOption; onSort: (s: SortOption) => void; showMobileFilters: () => void; activeFilterCount: number
}) {
  const [open, setOpen] = useState(false)
  const current = SORT_OPTIONS.find((o) => o.value === sort)
  return (
    <div className="flex items-center justify-between mb-6">
      <button onClick={showMobileFilters}
        ...
        {activeFilterCount > 0 && (
          <span className="w-4 h-4 rounded-full bg-brand-accent text-brand-black text-[9px] font-medium flex items-center justify-center">{activeFilterCount}</span>
        )}
      </button>
            ...
            <motion.div initial={{ opacity:0, y:6 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:6 }} transition={{ duration:0.18 }}
              ...
              {SORT_OPTIONS.map((opt) => (
                <button key={opt.value} onClick={() => { onSort(opt.value); setOpen(false) }}
                  className={clsx('block w-full text-left px-5 py-2.5 text-[11px] tracking-wide transition-colors',
                    sort === opt.value ? 'text-brand-accent bg-brand-accent/5' : 'text-brand-gray hover:text-brand-white'
                  )}>
                  {opt.label}
                </button>
              ))}
...
function PageHeader({ title, count }: { title: string; count: number }) {
  return (
    <div className="border-b border-brand-accent/12 mb-12 pb-10">
      <p className="eyebrow mb-3">BrimBT Design</p>
      <div className="flex items-end justify-between">
        <h1 className="font-display text-[clamp(36px,5vw,64px)] font-light text-brand-white leading-none">{title}</h1>
        <span className="text-[12px] text-brand-gray mb-2">{count} products</span>
      </div>
    </div>
  )
}

// ── 9. INNER PAGE (needs Suspense) ─────────────────────────
...
function ProductsPageInner() {
  ...
  const filteredProducts = useMemo(() => applyFilters(ALL_PRODUCTS, filters), [filters])
  ...
  const handleClearFilters = useCallback(() => {
    setFilters(DEFAULT_FILTERS); setPage(1)
  }, [])
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-6 lg:px-15 py-16">
      <PageHeader title={pageTitle} count={filteredProducts.length} />
      <div className="flex gap-12">
        <div className="hidden lg:block">
          <FilterSidebar filters={filters} onChange={handleFilterChange} onClear={handleClearFilters} totalActive={activeFilterCount} />
        </div>
        <div className="flex-1 min-w-0">
          <SortBar sort={filters.sort} onSort={(s) => handleFilterChange({ ...filters, sort: s })} showMobileFilters={() => setMobile(true)} activeFilterCount={activeFilterCount} />
